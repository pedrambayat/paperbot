...
- **`api/slack/events.py` + `api/process.py`** (Vercel serverless, Events API): function A verifies the Slack signature (`slack_verify.py`), answers the handshake, and enqueues to QStash (`jobs.publish_job`) within Slack's 3s deadline; function B verifies the QStash signature (`jobs.verify_qstash_signature`) and runs `process_job` with `RedisSummaryStore`. Deployed via `vercel.json` + `requirements.txt` (serverless deps exclude `slack-bolt`; the worker uses `slack_sdk.WebClient`).
