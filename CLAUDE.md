...
1. **`link_detector.py`** — `detect_papers(text)` parses raw message text into `PaperRef`s. URL spans are masked out before bare-DOI scanning so a DOI inside a URL isn't double-counted. Recognizes arXiv, bioRxiv/medRxiv (by host), `doi.org`, generic DOIs, nature.com article slugs (→ `10.1038/<slug>`), and `.pdf` links directly. **Any other URL** (except `NON_PAPER_HOSTS` like slack/google/github) becomes a `PaperKind.WEBPAGE` candidate — a maybe-a-paper link that the retriever resolves later. This is what makes support journal-agnostic.
2. **`retriever.py`** — `PaperRetriever.retrieve(ref)` dispatches on `PaperKind` and produces a `RetrievedPaper`. **`resolve_ref(ref)` runs first for WEBPAGE candidates** (in `core`, before dedupe): it fetches the page and reads Google Scholar/Highwire citation meta tags (`extract_citation_meta`) to recover a DOI or `citation_pdf_url`, or resolves an Elsevier PII in cell.com/sciencedirect URLs to a DOI via Crossref (`_doi_from_pii`). Returns a concrete DOI/PDF ref, or `None` for non-papers (raises for known `JOURNAL_HOSTS` where a silent skip would confuse the poster). Each source then has a **graceful-degradation chain**: try to download the full-text PDF; if blocked, fall back (rxiv → JATS/XML full text → abstract-only; DOI → Unpaywall PDF → OpenAlex PDF → **publisher `citation_pdf_url`** → abstract-only). The publisher-PDF step is what fetches subscription full text from an institutional IP. `mode` records whether full text or abstract was obtained. Two HTTP clients: `self.client` (httpx) for JSON metadata APIs, and `self.browser` (a `curl_cffi` Chrome-impersonating session) for **all page fetches and full-text content downloads** — bioRxiv/medRxiv and many publishers serve pages/PDFs from behind a Cloudflare WAF that blocks plain HTTP clients on TLS fingerprint, so `_download_pdf`/`_download_jats_text`/`_fetch_citation_meta` must use `self.browser`. Meta scraped during resolution is cached in `self._page_meta` (keyed by DOI) so the publisher-PDF fallback doesn't re-fetch the page.
