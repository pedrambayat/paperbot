...
4. **`store.py` / `store_redis.py`** — dedup stores sharing a `get(canonical_id)->dict|None` / `save(paper, summary, channel, ts)` interface. `SummaryStore` is SQLite (Socket Mode); `RedisSummaryStore` is Upstash Redis over REST (serverless — stateless, no connection to pool). Dedupe key is `PaperRef.canonical_id`.
5. **`formatter.py`** — builds Slack Block Kit payloads (`slack_blocks`, `failure_blocks`, `duplicate_blocks`).
6. **`core.py`** — the framework-agnostic heart, shared by both run modes: `summarize_and_post()` (dedupe check → retrieve via a `paper_factory` lambda → summarize → post → save) and `process_job(job, ...)` (summarize every link + PDF in a job, posting failures inline). `events.py` turns a Slack event envelope into a job dict (`job_from_event`) or `None`, and handles the `url_verification` handshake — used identically by Socket Mode and the webhook.

**Two run modes, one core:**
- **`app.py`** (Socket Mode, console script `paperbot`): `create_app()` registers a `message` handler that calls `job_from_event` → `process_job`. `main()` calls `ensure_tls_certs()` first (sets `SSL_CERT_FILE` to certifi's bundle — `slack_sdk`'s `urllib` has an empty SSL store on the macOS python.org build → `CERTIFICATE_VERIFY_FAILED`).
- **`index.py`** (Vercel serverless, Events API): a single **Flask** WSGI app (Vercel's current Python runtime wants one ASGI/WSGI entrypoint, not multiple `api/*.py` handler files — `pyproject.toml` is in `.vercelignore` so Vercel doesn't auto-detect it as an installable app). Two routes: `POST /api/slack/events` verifies the Slack signature (`slack_verify.py`), answers the URL handshake, and enqueues to QStash (`jobs.publish_job`) within Slack's 3s deadline; `POST /api/process` verifies the QStash signature (`jobs.verify_qstash_signature`) and runs `process_job` with `RedisSummaryStore`. Deployed via `requirements.txt` (Flask + worker deps; **no `slack-bolt`** — the worker uses `slack_sdk.WebClient`).
  - **Fast-ack rule:** heavy worker imports (`openai`, `curl_cffi`, `slack_sdk`) are imported lazily inside `_run_job`, NOT at module top — a slow cold start makes the ack exceed 3s, so Slack re-delivers and you get duplicate summaries. Keep the events path's imports light.
  - **Dedup is an atomic `store.claim()`** (Redis `SET NX` with TTL / SQLite `INSERT OR IGNORE`) taken before any work in `summarize_and_post`, so concurrent deliveries can't double-post. A genuine re-post (record already has a `slack_ts`) gets the "already summarized" notice instead.
  - **QStash is multi-region with per-region tokens.** `publish_job(base_url=...)` is driven by the `QSTASH_URL` env var; US accounts must set `https://qstash-us-east-1.upstash.io` or publish 404s with "user not found in this region". bioRxiv/medRxiv full-text PDFs are blocked by Cloudflare from Vercel's datacenter IPs, so the serverless deploy degrades those to abstract-only (arXiv etc. still get full text).

`models.py` holds the shared dataclasses/enums (`PaperRef`, `RetrievedPaper`, `PaperSummary`, `PaperKind`, `RetrievalMode`) that flow through every stage. `config.py` centralizes env parsing in `Settings.from_env()` (`SLACK_APP_TOKEN` is optional — only Socket Mode needs it).
